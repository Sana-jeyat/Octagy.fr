...
export default function CourseDetailClient({ id }: { id: string }) {
  ...
  useEffect(() => {
    async function fetchFormation() {
      try {
        ...
        // Vérifie si l'API a renvoyé un statut valide
        if (json.status !== "success") {
          throw new Error("Erreur API");
        }

        // Récupère le cours
        setFormation(json.course);
      } catch (err) {
        console.error(err);
      } finally {
        setLoading(false);
  ...
  if (!formation) return <p className="p-10">Formation introuvable</p>;

  const duration = formation.duration
    ? `${Math.floor(formation.duration / 60)}h${
        formation.duration % 60 ? ` ${formation.duration % 60}min` : ""
      }`
    : "—";
      ...
      <section className="bg-gradient-to-br from-purple-600 via-blue-600 to-indigo-700 text-white py-12">
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          <nav className="flex items-center space-x-2 text-purple-200 mb-4 text-sm">
            ...
            <span>{formation.category || "Catégorie"}</span>
            <ChevronRight className="w-4 h-4" />
            <span className="text-white font-semibold">{formation.title}</span>
          </nav>

          <h1 className="text-4xl font-extrabold mb-3">{formation.title}</h1>
          ...
          <div className="flex flex-wrap items-center gap-5 mt-6 text-purple-200 text-sm font-medium">
            <span className="px-3 py-1 bg-purple-300 bg-opacity-30 rounded-full">
            ...
            <div className="flex items-center space-x-1">
              ...
              <span>Nombre d’étudiants bientôt disponible</span>
            </div>
          </div>
        </div>
      </section>
        ...
        <div>
          <div className="sticky top-20 bg-white rounded-xl shadow p-6 space-y-6">
            <CourseEnrollmentCard
              ...
              fiatPrice={formation.price}
              knoPrice={formation.financierPrice}
            />
...
//           fetch(`${process.env.NEXT_PUBLIC_APP_API_URL}/courses/${id}/full`, {
//             cache: "no-store",
//           }),
//           fetch(
//             `${process.env.NEXT_PUBLIC_APP_API_URL}/courses/${id}/table-of-contents`,
//             { cache: "no-store" }
//           ),
//         ]);
...
//   const totalSessions = groupedSections.reduce(
//     (acc, sec) => acc + sec.sessions.length,
//     0
//   );
...
//                       course.duration % 60 ? ` ${course.duration % 60}min` : ""
//                     }`
//                   : "8h"}
//               </span>
//             </div>
...
//                 Vous recevrez des iKNO en validant{" "}
//                 <strong>tous les quiz</strong> du cours.
//               </p>
//             </div>

...
const paginatedEmployees = employees.slice(
  ...
  useEffect(() => {
    const fetchEmployees = async () => {
      ...
      setLoading(true);
      try {
        const res = await axiosInstance.get(`/companies/${selectedCompany}/employees`);
  ...
  const handleAddEmployee = async () => {
    ...
    setLoading(true);
    try {
      await axiosInstance.post(`/companies/${selectedCompany}/employees`, formData);
      setShowModal(false);
      setFormData({
        firstName: '',
        lastName: '',
  ...
  const handleExportPDF = () => {
    ...
    employees.forEach((e, i) => {
      doc.text(`${e.firstName} ${e.lastName} - ${e.position || 'N/A'}`, 10, 20 + i * 10);
    });
      ...
      <table className="w-full text-sm">
        ...
        <tbody>
          {paginatedEmployees.map((emp) => (
            <tr key={emp.id} className="border-b hover:bg-purple-50/40 transition">
              ...
              <td className="py-3 px-4">{emp.birthDate ? new Date(emp.birthDate).toLocaleDateString() : '-'}</td>
              <td className="py-3 px-4">{emp.nationality || '-'}</td>
              <td className="py-3 px-4">{emp.socialSecurityNumber || '-'}</td>
    ...
    {employees.length > 0 && (
      <div className="flex items-center justify-center gap-6 mt-6">
        ...
        <span className="text-gray-700 font-medium">
          Page <span className="text-purple-600">{currentPage}</span> / {totalPages}
        </span>
